---
page:
  '$path': '/sites/academy/home/documentation/jahia/8_2/developer/cms-security/html-filtering'
  '$type': 'jnt:page'
  'jcr:title': HTML Filtering
...
:::info
The module doesn't have to be enabled on a site to work and start filtering HTML. As soon as the module is installed and started, it **will be applied** and fully working based on the global configuration embedded with the module.
:::
...
2.  **Global custom configuration** (medium priority)
    ...
    Purpose: Override default settings for all Jahia sites. Managed by administrators to customize HTML filtering rules.
...
All configuration files follow the same structure, with separate sections for the _EDIT_ and _LIVE_ workspaces.
...
The `formatDefinitions` section defines regular expression patterns with a unique name that can be referenced in rules:
...
You can expect two different types of behavior when sanitization is performed. [Block-level content](https://developer.mozilla.org/en-US/docs/Glossary/Block-level_content) will see the tags removed but the content will remain present. While other types of tags will see their content removed entirely.

For example `<p>hello</p>` will be sanitized in `hello`, while `<script>alert('hello')</script>` will be removed entirely.
...
- `skip`: Node types/properties to be excluded from filtering (`skip` takes precedence over `process`)
...
This is useful when [certain privileged users](/cms/{mode}/{lang}/sites/academy/home/documentation/jahia/8_2/developer/leveraging-jahia-backend-capabilities/permissions-and-roles-deep-dive.html 'Permissions and roles deep dive') need to include HTML markup that would normally be filtered out. For example:
...
:::info
The `allowedRuleSet` is mandatory and should contain at least one rule, while `disallowedRuleSet` is optional.
:::
...
1.  The current user doesn't have any permission defined in `skipOnPermissions`
2.  The property matches at least one pattern in `process`
3.  The property doesn't match any pattern in `skip`
4.  The property is defined as a RichText property in the Compact Node Definition (.cnd)
...
In this example, `myHTMLProperty` will be processed by HTML filtering because it has the `richtext` interface, while `willNotBeProcessed` will not be processed because it lacks this constraint.

### JSON Overrides and HTML Filtering

jContent supports CND overrides ([called JSON overrides](/cms/{mode}/{lang}/sites/academy/home/documentation/jahia/8_2/developer/extending-and-customizing-jahia-ui/navigation-menu---text-separator/content-definitions-overrides-json-samples.html 'Examples of content definition JSON overrides')) which allow you to override the CND constraints. For example, JSON Overrides can override property constraints to use a different field type when editing content in Jahia jContent edition UI.

It's important to note that HTML filtering is not able to process these JSON overrides, so they will be ignored by the filtering system. For example, consider this CND:
...
## Migrating from HTML Filtering v1 to v2
...
Version 2 introduces a three-tier configuration system designed to make updates safer while allowing for customization:
...
- The flat structure of v1 for attributes and elements has been reorganized into rule sets
...
By following these steps, you can successfully migrate from HTML Filtering v1.x to v2 and take advantage of the enhanced features and flexibility offered by the new version.
